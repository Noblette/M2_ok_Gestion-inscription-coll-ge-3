...
import { getInscriptions, getEleves, getClasses } from "../api/inscriptionsApi"; // ou imports séparés
              ...
              <tbody className="divide-y divide-slate-200">
                {inscriptions.map(i => (
                  <tr key={i._id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-slate-800">{i.classe?.nom}</td>
                    <td className="px-6 py-4 text-sm text-slate-600">{i.anneeScolaire}</td>
                    <td className="px-6 py-4 text-sm font-semibold text-emerald-600">
                      {i.frais ? i.frais.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ") + " Ar" : "-"}
                    </td>
